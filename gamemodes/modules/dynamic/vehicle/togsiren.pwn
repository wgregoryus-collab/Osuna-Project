...
{
	if(!strcmp(string,"1",true)) return 1;
	else if(!strcmp(string,"2",true)) return 2;
	else if(!strcmp(string,"3",true)) return 3;
...
CMD:elm(playerid, params[])
{
    ...
    if(GetFactionType(playerid) != FACTION_POLICE && GetFactionType(playerid) != FACTION_FIRE && GetFactionType(playerid) != FACTION_SHERIFF)
        return SendErrorMessage(playerid, "You are not a member of a faction Police,Sheriff, or Fire Department.");

    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
        return SendErrorMessage(playerid, "Must be in steering position to use this command!.");
...
CMD:togsiren(playerid, params[])
{
	...
	if(sscanf(params, "k<VehicleSiren>S()[128]", action, nextParams))
	{
		...
		SendSyntaxMessage(playerid, "Entity : 1 (roof)/2 (dashboard)/3 (mid)/off");
		return 1;
	}	
	switch(action) 
	{
