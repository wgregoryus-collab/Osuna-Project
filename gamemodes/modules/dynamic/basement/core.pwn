#define MAX_DYNAMIC_UNDERGROUND		50
...
Function:UndergroundCreated(index)
{
...
Function:Underground_Load()
{
	if(cache_num_rows())
	{
		for(new i = 0; i != cache_num_rows(); i++)
		{
            ...
            UndergroundData[i][underFaction] = cache_get_field_int(i, "Faction");

            UndergroundData[i][enterInt] = cache_get_field_int(i, "enterInterior");
            UndergroundData[i][enterVW] = cache_get_field_int(i, "enterVW");
            UndergroundData[i][exitInt] = cache_get_field_int(i, "exitInterior");
            UndergroundData[i][exitVW] = cache_get_field_int(i, "exitVW");
    	...
    	printf("*** [F:RP Database: Loaded] basement data loaded (%d count)", cache_num_rows());
	}
	return 1;
}
...
Underground_Create(playerid)
{
	...
	if((index = Iter_Free(Underground)) != cellmin)
	{
        ...
        UndergroundData[index][underFaction] = -1;
...
Underground_Delete(index)
{
	if(Underground_IsExists(index))
	{
        ...
        mysql_tquery(g_iHandle, sprintf("DELETE FROM `underground` WHERE `id` = '%d'", UndergroundData[index][underID]));
...
Underground_Save(index)
{
	...
	if(Underground_IsExists(index))
	{
		mysql_format(g_iHandle, query, sizeof query, "UPDATE `underground` SET `enterX`='%.4f',`enterY`='%.4f',`enterZ`='%.4f',`exitX`='%.4f',`exitY`='%.4f',`exitZ`='%.4f',`exitRZ`='%.4f', `Faction`='%d', `enterInterior`='%d', `enterVW`='%d', `exitInterior`='%d', `exitVW`='%d' WHERE id=%d", 
            ...
            UndergroundData[index][underFaction],
            UndergroundData[index][enterInt],
            UndergroundData[index][enterVW],
            UndergroundData[index][exitInt],
            UndergroundData[index][exitVW],
