...
Function:Object_Load() {
  ...
  printf("*** [GarudaPride Database: Loaded] object data loaded (%d count)", rows);
  return 1;
}

Object_Nearest(playerid)
{
	foreach (new i : Obj) if (IsPlayerInRangeOfPoint(playerid, 3, ObjData[i][oPos][0], ObjData[i][oPos][1], ObjData[i][oPos][2])) {
		if (GetPlayerVirtualWorld(playerid) == ObjData[i][oVw] && GetPlayerInterior(playerid) == ObjData[i][oInt])
		return i;
	}
...
CMD:co(playerid, params[]) {
  ...
  if (sscanf(params, "d", modelid))
    return SendSyntaxMessage(playerid, "/co [modelid]");
...
CMD:eop(playerid, params[]) {
  if (CheckAdmin(playerid, 5))
    return PermissionError(playerid);

  new opt[64], value[128];
  if (sscanf(params, "s[64]S()[128]", opt, value)) {
    SendSyntaxMessage(playerid, "/eop [option]");
...
CMD:doj(playerid, params[]) {
  if (CheckAdmin(playerid, 5)) 
    return PermissionError(playerid);

  new id;
  if (sscanf(params, "d", id))
    return SendSyntaxMessage(playerid, "/doj [object id]");
...
CMD:odl(playerid, params[])
{
	if (CheckAdmin(playerid, 1))
        return PermissionError(playerid);

	new obj_id = Object_Nearest(playerid);
	SendServerMessage(playerid, "Object Administrator ID: "YELLOW"%s", (obj_id == -1) ? ("NONE") : sprintf("%d", obj_id));
	return 1;
}
